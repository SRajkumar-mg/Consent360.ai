    ...
    <div>
      <div className="customer-panel">
        ...
        <div className="page-actions">
          ...
          <button className="btn btn-ghost" onClick={refresh} disabled={refreshing}><IconRefresh size={15} /> Refresh</button>
        </div>
      </div>

      <div className="metric-grid mb">
        <MetricCard label="Active Consents" value={count(summary, ['ACTIVE', 'GRANTED', 'RENEWED', 'UPDATED'])} icon={<IconCheck size={20} />} tone="success" sub={`of ${summary.consents.length} records`} />
        <MetricCard label="Pending" value={count(summary, ['PENDING', 'REQUESTED', 'NOT_REQUESTED'])} icon={<IconClock size={20} />} tone="warning" sub="awaiting action" />
        <MetricCard label="Denied" value={count(summary, ['DENIED'])} icon={<IconX size={20} />} tone="danger" sub="explicitly refused" />
        <MetricCard label="Withdrawn" value={count(summary, ['WITHDRAWN'])} icon={<IconAlert size={20} />} tone="danger" sub="revoked by principal" />
        <MetricCard label="Expired" value={count(summary, ['EXPIRED'])} icon={<IconHistory size={20} />} tone="slate" sub="past validity" />
        <MetricCard label="Expiring ≤ 30 days" value={summary.expiring_soon.length} icon={<IconShield size={20} />} tone="warning" sub="renew soon" />
      </div>

      {summary.expiring_soon.length > 0 && (
        <div className="card mb" style={{ borderColor: 'var(--warning)', background: 'var(--warning-soft)' }}>
            ...
            <div className="card-header">
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="text-xs text-muted" style={{ marginTop: 3 }}>
                  {g.purpose_code} · {active}/{total} active
                </div>
              ...
              <div className="flex">
                ...
                {canManage && allActive && (
                  <button className="btn btn-ghost-danger btn-sm" onClick={() => setConfirmWithdraw(g.consents[0])}>
      ...
      <FooterRow left={`Customer since ${formatDate(customer.created_at)}`} />
