...
import { Badge, MaskedValue, PageHead, StatCard, TableSkeleton, formatDate } from '../components/ui'
...
export function CustomersPage() {
  ...
  return (
    <div>
      <PageHead title="Customers" subtitle="Customer references synchronized from business applications — email and phone are masked for privacy" />

      <div className="stat-grid mb">
        <StatCard label="Customers" value={customers.length} icon={<IconUsers size={20} />} tone="primary" sub={search ? 'matching current search' : 'in consent directory'} />
        <StatCard label="Active" value={activeCount} icon={<IconShield size={20} />} tone="success" sub="enabled customer profiles" />
        <StatCard label="Source Apps" value={sources} icon={<IconUsers size={20} />} tone="purple" sub="connected business applications" />
      </div>
        ...
        {loading ? <TableSkeleton rows={6} cols={6} /> : (
          <div className="table-wrap">
            <table className="table">
              <thead>
                <tr><th>Customer</th><th>Email</th><th>Phone</th><th>Status</th><th>Source</th><th></th></tr>
              </thead>
              <tbody>
                {customers.map((c) => (
                  <tr key={c.id}>
      ...
      <div className="text-xs text-muted mt">Directory created: {customers[0] ? formatDate(customers[0].created_at) : '—'}</div>
    </div>
  )
}