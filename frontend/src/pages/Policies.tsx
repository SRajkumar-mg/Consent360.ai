...
import { Badge, ConfirmDialog, EmptyState, Modal, PageHead, Spinner, StatCard, useToast, formatDate } from '../components/ui'
  ...
  return (
    <div>
      ...
      <div className="stat-grid mb">
        <StatCard label={t.totalPolicies} value={policies.length} icon={<IconShield size={20} />} tone="primary" sub={t.totalPoliciesSub} />
        <StatCard label={t.activePolicies} value={policies.filter((p) => p.is_active).length} icon={<IconCheck size={20} />} tone="success" sub={t.activePoliciesSub} />
        <StatCard label={t.retiredPolicies} value={policies.filter((p) => p.status === 'RETIRED').length} icon={<IconTrash size={20} />} tone="danger" sub={t.retiredPoliciesSub} />
        <StatCard label={t.policyRules} value={totalRules} icon={<IconUsers size={20} />} tone="purple" sub={t.policyRulesSub} />
      </div>
            ...
            <div className="card-header">
              <div>
                ...
                <div className="text-xs text-muted" style={{ marginTop: 3 }}>
                  {t.defaultDecision} <b>{current?.default_decision}</b> · {t.currentVersion} <b>v{p.current_version}</b> · {rules.length} {t.rules}{rules.length === 1 ? '' : 's'}
                </div>
              </div>
              <div className="flex">
