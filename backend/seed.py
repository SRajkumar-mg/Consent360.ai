...
def is_production() -> bool:
    return get_settings().ENVIRONMENT.strip().lower() in ("production", "prod")
...
def seed(db: Session) -> None:
    ...
    if db.query(User).count() == 0:
        ...
        if is_production():
            # Structural guard (R2-12): never create demo users with known
            # passwords in production. Provision a single admin with a
            # random, must-rotate-on-first-login credential instead.
            import secrets

            if db.query(User).filter(User.username == settings.SEED_ADMIN_USERNAME).first() is None:
                temp_password = secrets.token_urlsafe(18)
                db.add(User(
                    username=settings.SEED_ADMIN_USERNAME,
                    full_name="System Administrator",
                    email="admin@consent.local",
                    email_search=hmac_digest("admin@consent.local"),
                    password_hash=hash_password(temp_password),
                    role_id=admin_role.id if admin_role else None,
                    is_active=True,
                ))
                print("[seed] Production mode: created admin with a randomly-generated "
                      f"password (see deployment secret store). Rotate on first login.")
            db.commit()
        else:
            users = [
    ...
    if is_production():
        # Structural guard (R2-12): never seed demo customers, consents, or
        # fabrication audit history in production. Reference data (roles,
        # purposes, categories, policies) above is required config and stays.
        db.commit()
        return
    ...
    if db.query(Customer).count() == 0:
        customers = [
            Customer(external_id="CUST-10001", external_id_search=hmac_digest("CUST-10001"),
                     name="Aarav Patel", email="aarav.patel@example.com", email_search=hmac_digest("aarav.patel@example.com"),
                     phone="+91-98111-22333", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10002", external_id_search=hmac_digest("CUST-10002"),
                     name="Sanya Iyer", email="sanya.iyer@example.com", email_search=hmac_digest("sanya.iyer@example.com"),
                     phone="+91-98222-33444", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10003", external_id_search=hmac_digest("CUST-10003"),
                     name="Vikram Rao", email="vikram.rao@example.com", email_search=hmac_digest("vikram.rao@example.com"),
                     phone="+91-98333-44555", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10004", external_id_search=hmac_digest("CUST-10004"),
                     name="Ananya Gupta", email="ananya.gupta@example.com", email_search=hmac_digest("ananya.gupta@example.com"),
                     phone="+91-98444-55666", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10005", external_id_search=hmac_digest("CUST-10005"),
                     name="Rohan Desai", email="rohan.desai@example.com", email_search=hmac_digest("rohan.desai@example.com"),
                     phone="+91-98555-66777", status="SUSPENDED", source_app="CRM_APP"),
            Customer(external_id="CUST-10006", external_id_search=hmac_digest("CUST-10006"),
                     name="Meera Krishnan", email="meera.k@example.com", email_search=hmac_digest("meera.k@example.com"),
                     phone="+91-98666-77888", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10007", external_id_search=hmac_digest("CUST-10007"),
                     name="Kabir Singh", email="kabir.singh@example.com", email_search=hmac_digest("kabir.singh@example.com"),
                     phone="+91-98777-88999", status="ACTIVE", source_app="CRM_APP"),
            Customer(external_id="CUST-10008", external_id_search=hmac_digest("CUST-10008"),
                     name="Ishita Bose", email="ishita.bose@example.com", email_search=hmac_digest("ishita.bose@example.com"),
