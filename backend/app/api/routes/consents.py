...
def list_consents(
    ...
    q = db.query(Consent).join(Customer, Consent.customer_id == Customer.id).order_by(Consent.customer_id, Consent.purpose_id)
    if scope:
        q = q.filter(Consent.source_app == scope)
    if customer_id:
        q = q.filter(Customer.external_id_search == hmac_digest(customer_id))
    if purpose_id:
...
def grant(consent_id: int, action: ConsentAction, db: Session = Depends(get_db),
...
def deny(consent_id: int, action: ConsentAction, db: Session = Depends(get_db),
         current_user: User = Depends(require_permission(PERM_CONSENT_MANAGE))):
    scope = get_org_scope(current_user)
    consent = _get_consent_or_404(db, consent_id, scope=scope)
...
def withdraw(consent_id: int, action: ConsentAction, db: Session = Depends(get_db),
             current_user: User = Depends(require_permission(PERM_CONSENT_MANAGE))):
    scope = get_org_scope(current_user)
    consent = _get_consent_or_404(db, consent_id, scope=scope)
...
def renew(consent_id: int, action: ConsentAction, db: Session = Depends(get_db),
          current_user: User = Depends(require_permission(PERM_CONSENT_MANAGE))):
    scope = get_org_scope(current_user)
    consent = _get_consent_or_404(db, consent_id, scope=scope)
