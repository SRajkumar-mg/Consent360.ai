...
def _create_evidence(
    ...
    *,
    ...
    # Resolve notice and legal document context
    notice = db.get(Notice, notice_id) if notice_id else None
    terms_doc = db.get(LegalDocument, notice.terms_document_id) if notice and notice.terms_document_id else None
    privacy_doc = db.get(LegalDocument, notice.privacy_document_id) if notice and notice.privacy_document_id else None

    # Compute content hash covering consent text + notice version + language
    content_payload = {
        "consent_text": consent_text or "",
        "notice_id": notice_id,
        "notice_hash": notice.content_hash if notice else "",
        "language": ctx.get("language", "en"),
        "terms_version": terms_doc.version if terms_doc else "",
        "privacy_version": privacy_doc.version if privacy_doc else "",
        "purpose_code": consent.purpose.code if consent.purpose else "",
    }
    content_hash = hashlib.sha256(json.dumps(content_payload, sort_keys=True, default=str).encode("utf-8")).hexdigest()
    ...
    evidence = ConsentEvidence(
        ...
        notice_id=notice_id,
        notice_hash=notice.content_hash if notice else "",
        terms_document_id=notice.terms_document_id if notice else None,
        privacy_document_id=notice.privacy_document_id if notice else None,
        terms_hash=terms_doc.content_hash if terms_doc else "",
        privacy_hash=privacy_doc.content_hash if privacy_doc else "",
        details={
            **(extra_metadata or {}),
            "purpose_code": consent.purpose.code,
    ...
    # R4-001: Generate evidence signature
    signable = {
        "tenant_id": consent.tenant_id,
        "data_principal_id": consent.customer.external_id if consent.customer else None,
        "purpose_id": consent.purpose_id,
        "decision": "GRANTED",
        "notice_id": notice_id,
        "notice_hash": notice.content_hash if notice else "",
    ...
    evidence.signature = sign_payload(signable)
