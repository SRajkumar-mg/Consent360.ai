...
class DataSharingEvent(Base):
    ...
    purpose_id: Mapped[int] = mapped_column(ForeignKey("purposes.id"), nullable=True)
    status: Mapped[str] = mapped_column(String(32), default="PENDING", index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)

    processor: Mapped["Processor"] = relationship("Processor", back_populates="data_sharing_events")
    customer: Mapped["Customer"] = relationship("Customer", back_populates="data_sharing_events")
