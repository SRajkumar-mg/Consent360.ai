...
export function App() {
  ...
  // A consent record needs a real data principal. The backend refuses a
  // fabricated visitor identity (gap B-04), and it is right to: a consent
  // attributed to visitor-<timestamp>@careerhub.local names nobody, so it
  // can neither be honoured nor withdrawn. Ask before opening the banner.
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Identity step - a consent needs someone to attribute it to */}
      {askIdentity && (
        <div className="consent-overlay" onClick={() => setAskIdentity(false)}>
          <div
            className="consent-banner"
            role="dialog"
            aria-modal="true"
            aria-labelledby="identity-title"
            aria-describedby="identity-desc"
            onClick={(e) => e.stopPropagation()}
            style={{ maxWidth: 460 }}
          >
            <div className="consent-header">
              <h2 id="identity-title">Manage your consent</h2>
              <p id="identity-desc">
                Your consent is recorded against you, so we need the email address you use
                with CareerHub. It lets you review or withdraw this consent later, and it is
                the only thing we ask for at this point.
              </p>
            </div>
            <form
              onSubmit={(e) => {
                ...
                consent.initConsent(email)
              }}
            >
              <div style={{ padding: '0 20px' }}>
                <label htmlFor="identity-email" style={{ display: 'block', fontWeight: 600, marginBottom: 6 }}>
                  Email address
                </label>
                <input
                  id="identity-email"
              ...
              <div className="consent-footer">
                <button type="button" className="btn btn-ghost" onClick={() => setAskIdentity(false)}>
                  Cancel
                </button>
      ...
      {/* Consent setup failed - surfaced instead of leaving the shield icon looking dead */}
      {!consent.loading && consent.error && (
        <div className="consent-overlay" onClick={consent.clearError}>
          <div className="consent-banner" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div className="consent-header">
              <h2>Couldn&apos;t load your consent preferences</h2>
              <p>{consent.error}</p>
            </div>
            <div className="consent-footer">
              <button className="btn btn-ghost" onClick={consent.clearError}>Dismiss</button>
              <button className="btn btn-primary" onClick={() => { consent.clearError(); setAskIdentity(true) }}>Try again</button>
            </div>
          </div>
        </div>
      )}
