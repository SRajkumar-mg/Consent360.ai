...
import { consentApi, crmApi, getErrorMessage, noticeVersionFrom, publicApi } from '../api'
import { CookieBanner } from '../components/CookieBanner'
import { COOKIE_CONSENT_KEY, CRM_LANG_KEY } from '../languages'
import { getSessionId } from '../session'
import {
  COOKIE_BANNER_VERSION, OFFERED_PURPOSE_CODES, decisionFor, emitDecision, grantedPurposeCodes,
} from '../bannerEvents'
...
const COOKIE_TENANT_CODE = 'CRM_PORTAL'
...
export function UsersPage() {
  ...
  useEffect(() => {
    publicApi
      .purposes(COOKIE_TENANT_CODE)
      .then((res) => setNoticeVersion(noticeVersionFrom(res.data)))
  ...
  const persistPrefs = async (lang: string, categories: Record<string, boolean>, uiControlId: string) => {
    // R2-07: report the decision before the save. It carries no identifier, so
    // it is not evidence of this principal's consent - it is the denominator
    // for K-02/K-03/K-47, which have to count a decision that the save below
    // then fails to persist just as much as one that succeeds. Reporting it
    // afterwards would quietly drop exactly the failures worth knowing about.
    emitDecision({
    ...
    try {
      await consentApi.saveConsentPreferences(bannerCustomerId, {
        lang,
        categories,
        context: {
          language: lang,
  ...
  const acceptAllCookies = async (lang: string) => {
    ...
    await persistPrefs(lang, categories, 'accept-all')
    setBannerCustomerId(null)
  }
  ...
  const rejectAllCookies = async (lang: string) => {
    ...
    localStorage.setItem(COOKIE_CONSENT_KEY, JSON.stringify({ all: false, categories, at: Date.now() }))
    localStorage.setItem(CRM_LANG_KEY, lang)
    notifyConsentChanged()
    await persistPrefs(lang, categories, 'reject-all')
    setBannerCustomerId(null)
  }
  ...
  const saveCookiePrefs = async (lang: string, categories: Record<string, boolean>) => {
    ...
    localStorage.setItem(CRM_LANG_KEY, lang)
    notifyConsentChanged()
    await persistPrefs(lang, cats, 'save-preferences')
    setBannerCustomerId(null)
  }

  const openConsentPortal = async () => {
    if (!bannerCustomerId && !users.length) return
    const targetId = bannerCustomerId ?? users[0]?.id
    if (targetId == null) return
    try {
      const res = await consentApi.getConsentContext(targetId)
      window.location.href = res.data.consent_portal_url
    } catch (e) {
      setError(getErrorMessage(e))
    }
  }
      ...
      <nav className="jhub-nav">
        <div className="jhub-nav-inner">
          ...
          <div className="jhub-nav-links">
            ...
            <button className="jhub-nav-btn" onClick={openConsentPortal} title="Manage consent">
