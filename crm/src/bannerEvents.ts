 ...
 */
/**
 * The banner build identifier. It lives here rather than beside either
 * consumer because two of them need it - the component that paints the
 * banner (which reports the impression) and the page that saves the decision
 * (which reports the decision and stamps ClientContext.banner_version) - and
 * K-47 breaks the decision mix down BY banner version, so the two drifting
 * apart would split one banner's numbers across two versions.
 */
export const COOKIE_BANNER_VERSION = 'crm-cookie-banner-v1'

const TENANT_CODE = 'CRM_PORTAL'
const ENDPOINT = `/public/${TENANT_CODE}/banner-events`
const NONCE_KEY = 'crm_banner_nonce'
